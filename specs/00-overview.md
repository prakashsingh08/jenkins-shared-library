...
| 2 | [02-first-step-hello.md](02-first-step-hello.md) (+ [02a — how `hello()` works, the slow version](02a-how-hello-works-explained.md), [02b — questions & clarifications](02b-questions-and-clarifications.md), [02c — the step registry](02c-step-registry-and-missing-steps.md)) | `call()`, `@Library`, the `_`, registering a Global Pipeline Library in the Jenkins UI, reading library errors | `vars/hello.groovy` + a `hello-library-demo` pipeline job that prints it | **Done** |
...
| 14 | [14-parallel-and-build-speed.md](14-parallel-and-build-speed.md) | `parallel` in Declarative; building branches dynamically and the closure-capture bug; `stash`/`unstash` between agents; caching and timeouts; measuring before optimising | `vars/buildJavaP14.groovy` with parallel checks, plus a real before/after number | **Spec only** |
